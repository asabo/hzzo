...
/**
 * @author asabo
 * izmjenio 08.05.05. - asabo - dodana metoda find()
 */
public class Drzave extends CacheabilniDAO<DrzavaVO> implements DrzavaDAO
{
 ...
 /* 25.04.05. - asabo - za odredjeni redak u tablici TableModel poslat ce ValueObject 
  * koji se nalazi u tom retku, a metodi je zadatak vratiti vrijednost za zadanu kolonu
 ...
 public String getColumnName(int rb) {
	 if (rb<0 || rb>zaglavlja.length) return null;
	 else
	 return zaglavlja[rb];
 }
	...
	{
		//25.04.05. - asabo - u slucaju da nema podataka vratit ce se prazna lista
		// kako se ne bi dogodio kakav NullPointerException iznad. Zato bezuvjetno 
		// instanciramo listu na pocetku. Princip za sve findAll metode
		List<DrzavaVO> lista=new ArrayList<DrzavaVO>();
		
		String upit="select sifra,naziv,cc,cc3 from DRZAVE where status<>"+STATUS_DELETED;
		...
		java.sql.Statement st=null;
				...
				finally
				{
					...
					try{if (st!=null) st.close();}catch(SQLException sqle){} st=null;
					DAOFactory.freeConnection(c); c=null;
				}
		return lista;
	}//findAll
	...
	public void delete(Object obj) throws SQLException {
		...
		if (ulaz==null) throw new SQLException("ulazna vrijednost za drzavu je null!");
		
		String upit="update drzave set status='D' where sifra="+ulaz.getSifra();
		
		int komada=DAOFactory.performUpdate(upit);
	    

		if (komada!=1) 
		Logger.log("Neuspješno brisanje države sf:"+ulaz.getSifra().intValue(),null);
		else
	...
	public void insert(Object obj) throws SQLException {
		...
		DrzavaVO ulaz=(DrzavaVO)obj;
		if (ulaz==null) throw new SQLException("ulazna vrijednost je null!");
		
		boolean praznaTablica=DAOFactory.isTableEmpty("drzave");
		
		String upit="insert into drzave (sifra,naziv,status,cc,cc3) values ("+
		(praznaTablica?"1,":"(select max(sifra)+1 from drzave),")+ //07.12.05. -asabo- izmjenjeno
    " '"+ulaz.getNaziv()+"','U','"+ulaz.getCc()+"','"+ulaz.getCc3()+"')";
		
		int komada=DAOFactory.performUpdate(upit);
	    
		if (komada!=1) 
		Logger.log("Neuspješno unošenje države sf:"+ulaz.getSifra().intValue()+" naziv: "+ulaz.getNaziv(),null);

	}//insert

	/* (non-Javadoc)
	 * @see biz.sunce.dao.DAO#read(java.lang.Object)
	 * 25.11.05. asabo - ne znam koji je smisao ove metode, eto, pa neka ceka...
	 */
	public DrzavaVO read(Object obj) throws SQLException {
		...
		if (obj==null) throw new SQLException("ulazna vrijednost je null!");
		...
		String upit = "select sifra,naziv,cc,cc3 from DRZAVE where sifra="+sifra.intValue();
		...
		try {
			...
			try {if (rs != null)rs.close();} catch (SQLException sqle) {} rs=null;
			try {if (st != null)st.close();} catch (SQLException sqle) {} st=null;
			if (c!=null) DAOFactory.freeConnection(c); c=null;
		}		
		return drzava;		
	}//read

	/* (non-Javadoc)
	 * @see biz.sunce.dao.DAO#update(java.lang.Object)
	 * 25.04.05. asabo - nakon sto se updateira objekt u bazi podataka, njegovi se podaci mjenjaju, 
	 * dakle ulazni objekt se MIJENJA, ne kreira se novi. (nema smisla drzati stari i novi)
	 */
	public boolean update(Object obj) throws SQLException {
		DrzavaVO ulaz=(DrzavaVO)obj;
		if (ulaz==null) throw new SQLException("ulazna vrijednost je null!");
		
		String upit="update drzave set naziv='"+ulaz.getNaziv()+"', status='U', cc='"+ulaz.getCc()+"', cc3='"+ulaz.getCc3()+"' where sifra="+ulaz.getSifra();
		
		int komada=DAOFactory.performUpdate(upit);
	    
		if (komada!=1) 
		{
			Logger.log("Update CSC drzave sf:"+ulaz.getSifra().intValue()+" nije promjenio vrijednost");			
		}
		else
	...
	public Class getVOClass() throws ClassNotFoundException {
		return Class.forName("biz.sunce.opticar.vo.Drzava");
	}
