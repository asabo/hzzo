...
public final class Lijecnici implements LijecnikDAO{
	...
	public void insert(Object objekt) throws SQLException {
		...
		if (objekt==null)
			throw new SQLException("Insert lijecnik: ulazna vrijednost je null!");
		...
		String upit =
				" INSERT INTO "+tablica
					...
					+"	titula,"				//3
					+"	created,"
				        +"  created_by"  // 07.12.05. -asabo- dodano, treba imati podatak o tome tko je kreirao lijecnika
					+") VALUES ("+
					(sifra=DAOFactory.vratiSlijedecuSlobodnuSifruZaTablicu(tablica,"sifra"));

					upit+=",?,?,?,current_timestamp,?)";//,"+GlavniFrame.getSifDjelatnika()+")";	//4 komada
	...
	public boolean update(Object objekt) throws SQLException {
		...
		String upit =
					" UPDATE lijecnici SET "
					+"		ime=?,"						//1
					+"		prezime=?," 				//2
					+"		titula=?,"					//3
					+"		status=?,"				//4
					+"		updated=current_timestamp,"
					+"    updated_by="+GlavniFrame.getSifDjelatnika()  //07.12.05. -asabo- dodana i ta kolona - 28.05.05. -asabo- a tko je promjenio podatak?!?
					+" WHERE sifra=?";			//5
	...
	Hashtable<Integer, LijecnikVO> lijecniciCache=new Hashtable<Integer, LijecnikVO>(128);
	
	public LijecnikVO read(Object kljuc) throws SQLException {

		if (kljuc!=null && kljuc instanceof Integer && lijecniciCache.containsKey((Integer)kljuc))
		...
		{
			Logger.log("Lijecnici.read pozvani sa ulaznim parametrom klase: "+(ulazSifra!=null?ulazSifra.getClass().toString():"(null obj)"),null);
			return null;
		}
		...
		String upit=
					...
					+ " FROM "
					+ "			lijecnici"
					+ " WHERE status<>"+STATUS_DELETED+" and ";

		if (ulazSifra!=null){	upit += " sifra = "+ulazSifra.intValue();}
		else  //28.05.05. -asabo- ne mogu biti oba postavljena
		if (ulazIme!=null)	{ upit += " ime like '%"+ulazIme+"%'";}
		else
		if (krit!=null && krit.getKriterij().equals(KRITERIJ_IME_PREZIME))
		{
			ArrayList l=(ArrayList)krit.getPodaci();
			String ki=(String)l.get(0);
			String kp=(String)l.get(1);
			if (ki!=null) ki=ki.replaceAll("\\'","").toLowerCase();
			if (kp!=null) kp=kp.replaceAll("\\'","").toLowerCase();
	...
	public List<LijecnikVO> findAll(Object kljuc) throws SQLException {
		String upit=
				   ...
				   + " FROM "
				   + "			lijecnici"  //15.05.05. -asabo- bilo je djelatnici, promjenio u lijecnici
                ...
                if (kljuc!=null && kljuc instanceof String)
                {
                    ...
                    // ako u filteru ima razmak onda se vjerojatno trazi kombinacija ime-prezime...
                    String [] fspl=filter.split(" ");
                    if (fspl!=null && fspl.length>1)
                    {
                     upit+=" and (lower(ime) like '%"+fspl[0]+"%' and lower(prezime) like '%"+fspl[1]+"%') order by prezime,ime";
                    }
                    else
                    upit+=" and (lower(ime) like '%"+filter+"%' or lower(prezime) like '%"+filter+"%') order by prezime,ime";
                    }//if
                }//if kljuc != null
...
//	----------------------------Metode vezae uz prikaz----------------------------
	public Class getColumnClass(int columnIndex) {
		try{
		 switch(columnIndex){
			case 0:									return Class.forName("java.lang.Integer");
			case 1:	case 2:	case 3:	return Class.forName("java.lang.String");//Oba case-a vracaju String
			default:	return null;
		 }//switch
		}
	...
	public Object getValueAt(LijecnikVO vo, int kolonas) {
		if (vo==null) return null;
			...
			switch(kolonas){
				case 0:		return lijecnik.getSifra();
				case 1:		return lijecnik.getIme();
				case 2:		return lijecnik.getPrezime();
				case 3:		return lijecnik.getTitula();
				default:	return null;
			}
  }//getValueAt
	...
	private LijecnikVO constructLijecnik(ResultSet rs) throws SQLException {
		...
		l.setLastUpdated(rs.getTimestamp("UPDATED")!=null?rs.getTimestamp("UPDATED").getTime():0L);
	...
	public GUIEditor getGUIEditor() {
		try {
			return (GUIEditor)Class.forName(DAO.GUI_DAO_ROOT+".Lijecnik").newInstance();
		}
		catch(InstantiationException ie)
		{
			Logger.log("InstantiationException kod povlacenja GUIEditora klase Lijecnik",ie);
			return null;
		}
		catch(IllegalAccessException iae)
		{
			Logger.log("IllegalAccessException kod povlacenja GUIEditora klase Lijecnik",iae);
			return null;
		}
